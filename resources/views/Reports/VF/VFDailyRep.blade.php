        ...
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-VFReport" role="tabpanel"
                    ...
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link active " id="nav-VFReport-tab" data-toggle="tab"
                               href="#nav-VFReport" role="tab" aria-controls="nav-VFReport"
                ...
                <section>
                    <div class="row">
                        <div class="col-12">
                            <div class="table-container" id="VFReportTableContainer">
                                <table class="table table-bordered table-striped" id="VFReportTable" width="120%"
                                       cellspacing="0">
                                    ...
                                    <tr class="text-white">
                                        ...
                                        <th>Defauts</th>
                                        <th>Ovalisation</th>
                                        <th>OrthogonaliteD</th>
                                        <th>OrthogonaliteF</th>
                                        <th>Rectitude</th>
                                        <th>ChanfreinD</th>
                                        <th>ChanfreinF</th>
                                        <th>Observation</th>
                                    ...
                                    @if(isset($reports))
                                        ...
                                        @foreach($reports as $item)
                                            <tr id="report{{$item->Id}}" rapportEtat="{{$item->Etat}}"
                                                ...
                                                <td>{{$item->Poids}}</td>
                                                <td>{{$item->Defauts}}</td>
                                                <td>{{$item->Ovalisation}}</td>
                                                <td>{{$item->OrthogonaliteD}}</td>
                                                <td>{{$item->OrthogonaliteF}}</td>
                                                <td>{{$item->Rectitude}}</td>
                                                <td>{{$item->ChanfreinD}}</td>
                                                <td>{{$item->ChanfreinF}}</td>
...
@section('script')
    ...
    <script>
        ...
        var table = $('#VFReportTable').DataTable({
        ...
        $(document).ready(function () {
            ...
            $('.reportEdit').click(function () {
                ...
                var win = window.open("{{url('/VisuelFinal/')}}/" + id, '_blank');
                if (win) {
        ...
        function addActions() {
            ...
            $("#Reports TR, #ArretReports TR").each(function () {
                ...
                $(this).mouseenter(function () {
                    ...
                    var height = ((($(this).height()) - $('#tr-actions').height()) / 2) + $('#VFReportTable_wrapper .row').height() + 5;
                    var width = (($('#VFReportTableContainer').width() - $('#tr-actions').width()) / 2);
                ...
                success: function (result) {
                    ...
                    if (result.reports.length > 0) {
                        result.reports.forEach(function (item) {
                            $('#VFReportTable').DataTable({ "lengthMenu": [[ -1,10, 25, 50], ["All",10, 25, 50]]}).row.add([
                                'Poste ' + item.Poste,
                                item.Machine,
                                item.Tube,
                                +item.Bis,
                            ...
                            $('#VFReportTable tbody tr:last-child')
